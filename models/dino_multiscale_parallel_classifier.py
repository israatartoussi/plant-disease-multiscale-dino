...
class DinoEncoder(nn.Module):
    """
    Extract intermediate DINOv3 features from layers 3,6,9,12
    using 0-based indices (2,5,8,11).
    """
    def __init__(
        self,
...
# =========================
# OLD Gated CNN (the successful one)
# =========================
...
class CrossGateAttention(nn.Module):
    """
    Gate one scale using context from the other scales.
    """
...
class CrossFusion(nn.Module):
    """
    S1 gated by (S2+S3+S4), etc.
    """
    def __init__(self, channels: int):
        super().__init__()
        self.g1 = CrossGateAttention(channels)
        self.g2 = CrossGateAttention(channels)
        self.g3 = CrossGateAttention(channels)
        self.g4 = CrossGateAttention(channels)

    def forward(self, feats):
        S1, S2, S3, S4 = feats

        O1 = self.g1(S1, S2 + S3 + S4)
        O2 = self.g2(S2, S1 + S3 + S4)
        O3 = self.g3(S3, S1 + S2 + S4)
        O4 = self.g4(S4, S1 + S2 + S3)
...
class DinoMultiScaleClassifier(nn.Module):
    ...
    For each scale:
        Gi = GatedCNN(Si)
        Ci = CrossGated(Si, other scales)
        Fi = Gi + Ci

    Then:
        F1,F2,F3,F4 -> Concat -> CBAM -> Classifier
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # branch 1: old gated cnn
        gated_maps = [
            self.gated_blocks[i](maps[i]) for i in range(self.num_scales)
        ]

        # branch 2: cross-gated between scales
        cross_maps = self.cross_fusion(maps)

        # parallel fusion per scale
        fused_scales = [
            gated_maps[i] + cross_maps[i] for i in range(self.num_scales)
        ]

        # final fusion
        fused = torch.cat(fused_scales, dim=1)
