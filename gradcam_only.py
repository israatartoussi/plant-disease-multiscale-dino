...
def pick_best_model(dataset):
    """اختر الأفضل من reports/tables/best_models.json؛
       ولو مفقود/متعادل، نحسب من test_metrics.json مع أولوية TIE_ORDER."""
    ...
    # fallback: اقرأ test_acc لكل النماذج
    scores = {}
    for m in ZOO.keys():
    ...
    tied = [m for m,v in scores.items() if abs(v - best_val) < 1e-9]
...
class SimpleGradCAM:
    def __init__(self, model, layer):
        ...
        self.h2 = layer.register_full_backward_hook(self._bwd)  # compat torch>=1.8
    ...
    def __call__(self, x):
        ...
        logit = self.m(x)
        cls = logit.argmax(1)
        onehot = torch.zeros_like(logit).scatter_(1, cls.view(-1,1), 1.0)
        (logit*onehot).sum().backward(retain_graph=True)
...
def overlay(img_tensor, cam, alpha=0.35):
    ...
    out = np.clip((1-alpha)*img + alpha*heat, 0, 1)
    return out

def make_pair_figure(img, ovl_best, ovl_base, title_left, title_right, dpi, out_pdf, out_png):
    plt.figure(figsize=(8,3.2), dpi=dpi)  # صف واحد عمودان
    for i,(arr,title) in enumerate([(ovl_base, title_left),(ovl_best, title_right)]):
        ax = plt.subplot(1,2,i+1)
        ax.imshow(arr); ax.set_title(title, fontsize=10)
        ax.axis("off")
...
def main():
    ...
    ap.add_argument("--dataset", help="اسم داتا-ست واحدة؛ إذا تُركت فارغة يستخدم --all")
    ap.add_argument("--all", action="store_true", help="شغّل على كل الداتا-سِت")
    ap.add_argument("--n", type=int, default=1, help="عدد الصور لكل داتا-ست (افتراضياً 1)")
    ...
    if args.all:
        datasets = [p.name for p in Path("data").iterdir() if (p/"test").exists() and (p/"classes.txt").exists()]
        datasets.sort()
    else:
        if not args.dataset:
    ...
    for d in datasets:
        ...
        # نفس التحويلات اللي استُخدمت للتدريب/التقييم
        tfm = transforms.Compose([transforms.Resize((224,224)), transforms.ToTensor()])
        ds  = datasets_mod := datasets  # silence linter
        ds  = datasets.ImageFolder((Path("data")/d/"test").as_posix(), tfm)
        if len(ds) == 0:
            print(f"[SKIP] empty test split: {d}")
            continue
        idxs = sorted(random.sample(range(len(ds)), k=min(args.n, len(ds))))

        # Grad-CAM مُبسّط على آخر conv
        ...
        for i,idx in enumerate(idxs, start=1):
            ...
            make_pair_figure(img, ovl_f, ovl_b, title_l, title_r, args.dpi, out_pdf, out_png)
