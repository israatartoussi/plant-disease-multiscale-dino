...
from models.dino_multiscale_cross_gated_classifier import DinoMultiScaleClassifier
...
def build_loaders(data_root: str, img_size: int, batch_size: int, num_workers: int = 0):
    root = Path(data_root)
    ...
    tr = DataLoader(ds_train, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin)
    va = DataLoader(ds_val, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin)
    te = DataLoader(ds_test, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin)
...
def main():
    ...
    ap.add_argument("--out", default="runs_dino_multiscale_cross_gated")
    args = ap.parse_args()
    ...
    out_dir = Path(args.out) / args.dataset / "dino_multiscale_cross_gated"
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(
            f"[E{ep}] tr_loss={tr_loss:.4f} | tr_acc={tr_acc:.3f} | "
